...
#%% 
    ...
    placefields, binsxy = nap.compute_2d_tuning_curves(group = pyr2, feature = position[['x', 'z']], ep = ep, nb_bins=20)                                               
    
    spatialinfo = nap.compute_2d_mutual_info(placefields, position[['x', 'z']], ep = ep)
...
#%%  Place field stability 
    ...
    # halves = nap.IntervalSet( start = [position.restrict(epochs['wake'].loc[[0]]).time_support.start[0], center.t[0]],
    #                          end = [center.t[0], position.restrict(epochs['wake'].loc[[0]]).time_support.end[0]])
    ...
    # pf1, binsxy = nap.compute_2d_tuning_curves(group = pyr2, feature = position[['x', 'z']], ep = half1, nb_bins=20)  
    # pf2, binsxy = nap.compute_2d_tuning_curves(group = pyr2, feature = position[['x', 'z']], ep = half2, nb_bins=20)  
...
#%% Plot tracking 
    ...
    # if name != 'B2618':
        plt.figure()
        plt.subplot(121)
        plt.plot(position['x'], position['z'], color = 'grey')
        spk_pos1 = pyr2[pyr2.index[10]].value_from(position)

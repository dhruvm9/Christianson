...
data_directory = '/media/DataDhruv/Recordings/Christianson/AAVCAG_Cohort1/Recall'
...
for r, s in enumerate(datasets):
    ...
    # If recall phase, analyze only the first 3 min (180s) of task. Each video has 60 frames per second.
    # if taskphase == '2':
    trackedpos = trackedpos[0:60*180]
    likelihoods = likelihoods[:,0:60*180]
...
### COHORT3 MALE
    ...
    if sex == 'M':
        ...
        if dispobj == 'right':    
            ...
            rectR_inner = patches.Rectangle((x_objR - 55, y_objR - 105), 88, 147, linewidth=1, edgecolor='g', facecolor='none')
            rectR = patches.Rectangle((x_objR - 76.5, y_objR - 126.5), 131, 190, linewidth=1, edgecolor='b', facecolor='none')
        
        elif (dispobj == 'left') and (mousename != '743'): 
            
            rectL_inner = patches.Rectangle((x_objL - 40, y_objL - 106), 85, 150, linewidth=1, edgecolor='g', facecolor='none')
            rectL = patches.Rectangle((x_objL - 61.5, y_objL - 127.5), 128, 193, linewidth=1, edgecolor='b', facecolor='none')
            ...
            rectL_inner = patches.Rectangle((x_objL - 45, y_objL - 106), 85, 150, linewidth=1, edgecolor='g', facecolor='none')
            rectL = patches.Rectangle((x_objL - 66.5, y_objL - 127.5), 128, 193, linewidth=1, edgecolor='b', facecolor='none')

...
#%% Filter LFP in ripple band, then restrict to ripple periods
    ...
    # pls = pls.rolling(window = 8,win_type='gaussian',center=True,min_periods=1).mean(std=80)
    ...
    # prs = pls.rolling(window = 8,win_type='gaussian',center=True,min_periods=1).mean(std=80)
...
#%% Compute mean attenuation index for a session
    ...
    att_index = np.mean(ref_lfp.values/rip_lfp.values)
           
    # evt_att = []
    
    # for i in range(len(rip_ep)):
    #     evt_att.append(power_ref.restrict(rip_ep.loc[[i]]).values / power_lfp.restrict(rip_ep.loc[[i]]).values)
...
# wt2 = np.array(['WT' for x in range(len(sess_att_wt))])
# ko2 = np.array(['KO' for x in range(len(sess_att_ko))])    
...
# s2 = pd.DataFrame(data = [evt_all, genotype], index = ['att', 'genotype']).T
